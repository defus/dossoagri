...
class AgriculteurController extends \BaseController {
    ...
    public function select2(){
      ...
      $search = \Input::get('q');
      $order = \Input::get('order', 'Nom');
      
      $query = Agriculteur::getQuery();
      $total = $query->count();
      if($search != ''){
        $query->where(function($q) use($search){
          $q->where(DB::raw('LOWER(Nom)'), 'LIKE', Str::lower('%' . trim($search) . '%' ));
          $q->orwhere(DB::raw('LOWER(Prenom)'), 'LIKE', Str::lower('%' . trim($search) . '%' ));
		  $q->orwhere(DB::raw('LOWER(Username)'), 'LIKE', Str::lower('%' . trim($search) . '%' ));
        });
      }
